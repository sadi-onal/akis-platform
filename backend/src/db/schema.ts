...
export const planCandidateStatusEnum = pgEnum('plan_candidate_status', [
  'unbuilt',
  'queued',
  'building',
  'built',
...
export const threadTaskStatusEnum = pgEnum('thread_task_status', [
  'pending',
  'awaiting_user_input',
  'answered',
  'completed',
...
export const jobs = pgTable(
  ...
  (table) => ({
    ...
    typeStateCreatedIdx: index('idx_jobs_type_state_created').on(
      table.type,
      table.state,
      table.createdAt
    ),
 ...
 */
export const jobTraces = pgTable(
  'job_traces',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    jobId: uuid('job_id')
 ...
 */
export const jobAiCalls = pgTable(
  'job_ai_calls',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    /**
     * Legacy single-agent path (`/api/agents/*`) sets this; pipeline rows leave
     * it NULL. Always reads via FK so cascades handle cleanup.
     */
    jobId: uuid('job_id').references(() => jobs.id, { onDelete: 'cascade' }),
    /**
     * T1: Pipeline orchestrator path sets this; legacy rows leave it NULL.
     * AiCallsService prefers this column when present.
     */
    pipelineId: uuid('pipeline_id').references(() => pipelines.id, { onDelete: 'cascade' }),
    /** Call index within the job/pipeline (0-based, for ordering) */
  ...
  (table) => ({
    ...
    pipelineIdIdx: index('idx_job_ai_calls_pipeline_id').on(table.pipelineId),
  })
);
...
export const conversationThreads = pgTable(
  ...
  (table) => ({
    ...
    userStatusUpdatedIdx: index('idx_conversation_threads_user_status_updated').on(
      table.userId,
      table.status,
      table.updatedAt
    ),
...
export const conversationMessages = pgTable(
  ...
  (table) => ({
    threadCreatedIdx: index('idx_conversation_messages_thread_created').on(
      table.threadId,
      table.createdAt
    ),
    userCreatedIdx: index('idx_conversation_messages_user_created').on(
      table.userId,
      table.createdAt
    ),
  })
 ...
 */
export const planCandidates = pgTable(
  'plan_candidates',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    threadId: uuid('thread_id')
  ...
  (table) => ({
    threadStatusUpdatedIdx: index('idx_plan_candidates_thread_status_updated').on(
      table.threadId,
      table.status,
      table.updatedAt
    ),
 ...
 */
export const planCandidateBuilds = pgTable(
  'plan_candidate_builds',
  {
    ...
    threadId: uuid('thread_id')
      .notNull()
      .references(() => conversationThreads.id, { onDelete: 'cascade' }),
    planCandidateId: uuid('plan_candidate_id')
      .notNull()
      .references(() => planCandidates.id, { onDelete: 'cascade' }),
    userId: uuid('user_id')
      .notNull()
      .references(() => users.id, { onDelete: 'cascade' }),
  ...
  (table) => ({
    threadCreatedIdx: index('idx_plan_candidate_builds_thread_created').on(
      table.threadId,
      table.createdAt
    ),
    candidateCreatedIdx: index('idx_plan_candidate_builds_candidate_created').on(
      table.planCandidateId,
      table.createdAt
    ),
  })
);
 ...
 */
export const threadTrustSnapshots = pgTable(
  ...
  (table) => ({
    threadCreatedIdx: index('idx_thread_trust_snapshots_thread_created').on(
      table.threadId,
      table.createdAt
    ),
    userCreatedIdx: index('idx_thread_trust_snapshots_user_created').on(
      table.userId,
      table.createdAt
    ),
  })
...
export const skills = pgTable(
  ...
  (table) => ({
    ...
    profileSkillUnique: uniqueIndex('idx_skills_profile_name_unique').on(
      table.profileId,
      table.name
    ),
  })
...
export const jobPosts = pgTable(
  ...
  (table) => ({
    ...
    sourceExternalUnique: uniqueIndex('idx_job_posts_source_external_unique').on(
      table.sourceId,
      table.externalId
    ),
...
export const matches = pgTable(
  ...
  (table) => ({
    ...
    profileJobUnique: uniqueIndex('idx_matches_profile_job_unique').on(
      table.profileId,
      table.jobPostId
    ),
  })
...
export const oauthAccounts = pgTable(
  ...
  (table) => ({
    ...
    providerAccountIdx: index('idx_oauth_accounts_provider_account').on(
      table.provider,
...
export const userAiKeys = pgTable(
  ...
  (table) => ({
    userProviderUnique: uniqueIndex('idx_user_ai_keys_user_provider').on(
      table.userId,
      table.provider
    ),
...
export const integrationCredentials = pgTable(
  ...
  (table) => ({
    userProviderUnique: uniqueIndex('idx_integration_creds_user_provider').on(
      table.userId,
      table.provider
    ),
...
export const agentConfigs = pgTable(
  'agent_configs',
  {
    ...
    // PR behavior
    prTitleTemplate: varchar('pr_title_template', { length: 500 }).default(
      'docs(scribe): update {path}'
    ),
...
export const crewRuns = pgTable(
  'crew_runs',
  {
    ...
    workerRoles: jsonb('worker_roles')
      .$type<
        Array<{
          role: string;
          agentType: string;
    ...
    coordinatorJobId: uuid('coordinator_job_id').references(() => jobs.id, {
      onDelete: 'set null',
    }),
...
export const crewTasks = pgTable(
  'crew_tasks',
  {
    ...
    crewRunId: uuid('crew_run_id')
      .notNull()
      .references(() => crewRuns.id, { onDelete: 'cascade' }),
    title: varchar('title', { length: 500 }).notNull(),
    description: text('description'),
 ...
 */
export const crewMessages = pgTable(
  'crew_messages',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    crewRunId: uuid('crew_run_id')
...
export const knowledgeSources = pgTable(
  'knowledge_sources',
  {
    ...
    contentHash: varchar('content_hash', { length: 64 }),
    verificationStatus: knowledgeVerificationStatusEnum('verification_status')
      .notNull()
      .default('unverified'),
 ...
 */
export const chatRetrievalAnchors = pgTable(
...
export const devSessionStatusEnum = pgEnum('dev_session_status', ['active', 'paused', 'closed']);
...
export const devSessions = pgTable(
  'dev_sessions',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const devMessages = pgTable(
  'dev_messages',
  {
    ...
    sessionId: uuid('session_id')
      .notNull()
      .references(() => devSessions.id, { onDelete: 'cascade' }),
...
export type {
  AgentActivityInsert,
  AgentActivitySelect,
} from '../pipeline/db/agent-activity-schema.js';
