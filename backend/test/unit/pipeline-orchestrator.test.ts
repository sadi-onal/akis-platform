...
// PDP-3 B4: these tests predate the preview-confirm gate and assert the
// "Proto → Trace → completed" legacy flow. Opt back into auto-push so the
// orchestrator does not halt at `awaiting_push_confirm`. Must run BEFORE
// config/env.ts is imported (transitively via orchestrator).
