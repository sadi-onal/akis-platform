...
- [ ] **1. Confirm preconditions.** The previous doc in numerical order is `approved` (check `.claude/state/pdp-progress.json`). Exception: `00-README.md` and `05-findings.md` can be drafted out of order — `00` is umbrella, `05` is gap analysis and benefits from late-cycle context. The orchestrator confirms or asks the user.
...
- [ ] **6. Mark approved.** Main session updates `.claude/state/pdp-progress.json#docs.<docId>.status` to `approved`, records `approved_at` timestamp, and the user's approval note (verbatim, even if just "ok").
