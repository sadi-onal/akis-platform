...
| **İlgisiz** — tamamen farklı alan | `.Codex/state/inbox.md`'ye 1 satır not ekle, dokunma |

**Inbox kuralları**: `.Codex/state/inbox.md` append-only, her entry `- [ ] {concern} — {context} — {tarih}` formatında. Session başında inbox kontrol edilir; doluysa kullanıcıyla öncelik belirlenir.

The T1-T5 series is the model to follow for parallel concerns.

## Multi-agent file ownership

Codex and Claude Code both work in this repository. Tool-specific support files are first-class repo files when needed, but do not overwrite the other tool's files.

- Claude Code owns Claude-specific files such as `.claude/` and `.agents/` unless the user explicitly asks Codex to edit them.
- Codex owns Codex-specific files such as `.Codex/` and Codex session notes.
- If a change seems useful in another tool's owned files, open or propose a GitHub issue instead of editing those files directly.
- Before pushing any branch, ask the user for approval. Do not push automatically.

## Conventions worth respecting

- Auto-formatting on edit is wired via `.Codex/hooks/format-on-edit.sh` (Prettier on `backend/` and `frontend/` files). Don't fight it — write code, let the hook normalize.
