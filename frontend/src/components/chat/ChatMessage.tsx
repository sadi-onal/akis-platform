  ...
  for (const seg of segments) {
    if (seg.startsWith('```')) {
      ...
      parts.push(
        <pre
          key={key++}
          className="my-2 overflow-x-auto rounded-lg bg-ak-surface-2 p-3 text-xs font-mono text-ak-text-primary"
        >
          {lang && (
            <span className="mb-1 block text-[10px] font-semibold uppercase text-ak-text-tertiary">
              {lang}
            </span>
          )}
          <code>{code}</code>
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = inlineRegex.exec(line)) !== null) {
          ...
          if (m[2])
            nodes.push(
              <strong key={key++} className="font-semibold text-ak-text-primary">
                {m[2]}
              </strong>
            );
          else if (m[3])
            nodes.push(
              <code key={key++} className="rounded bg-ak-surface-2 px-1 py-0.5 text-xs font-mono">
                {m[3]}
              </code>
            );
...
const AGENT_COLORS: Record<AgentName, { bg: string; border: string; text: string; label: string }> =
  {
    scribe: {
      bg: 'bg-ak-scribe/10',
      border: 'border-ak-scribe/30',
      text: 'text-ak-scribe',
      label: 'Scribe',
    },
    proto: {
      bg: 'bg-ak-proto/10',
      border: 'border-ak-proto/30',
      text: 'text-ak-proto',
      label: 'Proto',
    },
    trace: {
      bg: 'bg-ak-trace/10',
      border: 'border-ak-trace/30',
      text: 'text-ak-trace',
      label: 'Trace',
    },
  };
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-0.5 flex items-center gap-2">
          <span className={cn('text-xs font-semibold', cc.text)}>
            {AGENT_COLORS[message.role].label}
          </span>
        ...
        <div className="space-y-2">
          {message.questions.map((q, i) => (
            <div key={q.id} className={cn('rounded-lg border p-3', cc.border, cc.bg)}>
              ...
              {q.reason && <p className="mt-1 text-xs text-ak-text-tertiary">{q.reason}</p>}
  ...
  return (
    <div className="flex justify-end animate-in fade-in slide-in-from-right-2 duration-200">
      <div className="max-w-[85%] rounded-2xl bg-ak-primary px-4 py-2.5 text-sm text-[color:var(--ak-on-primary,#fff)]">
        ...
        <span className="mt-1 block text-right text-[10px] text-[color:var(--ak-on-primary,#fff)] opacity-60">
          {formatTime(message.timestamp)}
        </span>
      </div>
      {previewIndex !== null && images[previewIndex] && (
        <UserImagePreviewModal image={images[previewIndex]} onClose={() => setPreviewIndex(null)} />
      )}
    </div>
  );
}
...
function UserImageThumbnails({
  ...
  return (
    <div
      className={cn(
        hasText ? 'mt-2' : '',
        isSingle ? 'flex' : 'grid grid-cols-2 gap-1.5 sm:grid-cols-3'
      )}
    >
      {images.map((img, i) => (
        <button
  ...
  switch (message.type) {
            ...
            <details className="mt-3">
              ...
              <ul className="mt-2 space-y-1">
                {message.testFiles.map((f) => (
                  <li
                    key={f.filePath}
                    className="flex items-center gap-2 font-mono text-xs text-ak-text-secondary"
                  >
                    ...
                    <div key={criterion} className="text-xs">
                      ...
                      {isCovered && (
                        <div className="ml-5 mt-0.5 space-y-0.5">
                          {files.map((tf) => (
                            <span key={tf} className="block font-mono text-ak-text-tertiary">
                              {tf}
                            </span>
                          ))}
                        </div>
          ...
          {message.failed > 0 && (
            <div className="mt-3 flex gap-2">
              {onRetry && (
                <button
                  onClick={onRetry}
                  className="rounded-lg border border-ak-trace/20 px-3 py-1.5 text-xs font-medium text-ak-trace hover:bg-ak-trace/10 transition-colors"
                >
                  🔄 Trace'e Düzelttir
                </button>
              )}
              {onSkip && (
                <button
                  onClick={onSkip}
                  className="rounded-lg border border-ak-border px-3 py-1.5 text-xs font-medium text-ak-text-tertiary hover:text-ak-text-secondary transition-colors"
                >
                  ⏭ Geç
                </button>
              )}
        ...
        <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-4 animate-in fade-in slide-in-from-left-2 duration-200 animate-shake-subtle">
          ...
          <div className="mt-3 flex gap-2 flex-wrap">
            ...
            {message.recoveryAction === 'edit_spec' && onReject && (
              <button
                onClick={onReject}
                className="rounded-lg border border-blue-500/20 px-3 py-1.5 text-xs font-medium text-blue-400 hover:bg-blue-500/10 transition-colors"
              >
                Spec'i Duzenle
              </button>
            )}
            ...
            {message.recoveryAction === 'reconnect_github' && (
              <a
                href="/settings"
                className="rounded-lg border border-amber-500/20 px-3 py-1.5 text-xs font-medium text-amber-400 hover:bg-amber-500/10 transition-colors inline-flex items-center gap-1"
              >
            ...
            {message.recoveryAction === 'start_over' && (
              <a
                href="/chat"
                className="rounded-lg border border-ak-primary/20 px-3 py-1.5 text-xs font-medium text-ak-primary hover:bg-ak-primary/10 transition-colors inline-flex items-center gap-1"
              >
        ...
        <div
          className={cn(
            ...
            isPartial
              ...
              : 'border-ak-primary/30 bg-ak-primary/5'
          )}
        >
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-sm text-ak-text-secondary">
            {message.repoUrl && (
              <span className="flex items-center gap-1.5">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-ak-primary hover:underline font-mono text-xs"
                >
            ...
            <span>
              {message.fileCount} dosya · {message.lineCount} satır
            </span>
            {message.testCount != null && (
              <span>
                🧪 {message.testCount} test
                {message.coverage ? ` · %${message.coverage} kapsam` : ''}
              </span>
            )}
          </div>
          ...
          {message.cloneCommand && (
            <div className="mt-3 rounded-lg bg-ak-surface-2 p-3">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 text-xs font-mono text-ak-text-secondary break-all">
                  {message.cloneCommand}
                </code>
                <button
    ...
    case 'critic_review': {
      ...
      return (
        <div
          className={cn(
            'rounded-xl border p-4 animate-in fade-in slide-in-from-left-2 duration-200',
            scoreBg
          )}
        >
          <div className="mb-2 flex items-center gap-2 flex-wrap">
            ...
            <span
              className={cn(
                ...
                message.approved ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'
              )}
            >
            ...
            <details className="group">
              ...
              <div className="mt-2 space-y-1.5">
                {message.findings.map((f, i) => (
                  <div
                    key={i}
                    className="rounded-lg border border-ak-border-subtle bg-ak-surface/60 p-2"
                  >
      ...
      return (
        ...
        >
          ...
          {!streaming && citations && citations.length > 0 && (
            <div data-testid="chat-qa-citations" className="mt-3 flex flex-wrap gap-1.5">
              {citations.map((c, i) => (
                <span
                  key={i}
                  title={c.excerpt}
                ...
                >
                  ...
                  {c.refKey ? (
                    <span className="ml-1 font-mono text-[10px] text-ak-text-tertiary">
                      · {c.refKey}
                    </span>
                  ) : null}
                </span>
          ...
          {!streaming && needsBuild && sourceMessage && onSuggestBuild && (
            <div className="mt-3 flex items-center gap-2">
              ...
              <span className="text-[10px] text-ak-text-tertiary">
                Tıklarsan yeni bir build başlatırız.
              </span>
            </div>
          )}
        </div>
                ...
                {f.mappedCriteria.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {f.mappedCriteria.map((c, ci) => (
                      <span
                        key={ci}
                        className="px-2 py-0.5 rounded-full text-xs bg-purple-500/10 text-purple-400"
                      >
