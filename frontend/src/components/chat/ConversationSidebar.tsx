...
export const ConversationSidebar = memo(function ConversationSidebar({
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter(
      (c) =>
        ...
        c.repoFullName.toLocaleLowerCase('tr').includes(q)
    );
  ...
  const grouped = useMemo(() => {
    ...
    const groups: { label: string; items: typeof filtered }[] = [
      { label: t('sidebar.group.today'), items: [] },
      { label: t('sidebar.group.yesterday'), items: [] },
      { label: t('sidebar.group.thisWeek'), items: [] },
      { label: t('sidebar.group.older'), items: [] },
    ];
    ...
    // `t` ref is stable across re-renders within the same locale; locale
    // changes trigger a full provider re-render so this memo will recompute.
  ...
  return (
    ...
    >
        ...
        {collapsed ? (
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col">
              <span className="text-xl font-extrabold leading-tight tracking-tight text-ak-primary">
                AKIS
              </span>
              <span className="text-[9px] font-semibold uppercase tracking-[0.2em] text-ak-text-tertiary">
                Platform
              </span>
            </div>
          </div>
      ...
      {!collapsed && (
        <div className="space-y-2 px-3 pb-2">
          ...
          <button
            ...
            className={cn(
              'flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-ak-border px-3 py-2 text-xs font-medium text-ak-text-secondary',
      ...
      {collapsed && (
        <div className="flex flex-col items-center gap-1 px-2 pb-2">
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-lg border border-dashed border-ak-border text-ak-text-secondary hover:border-emerald-400 hover:text-ak-primary transition-colors"
          >
            <svg
              className="h-3.5 w-3.5"
              fill="none"
          ...
          {/* PR-U2 #4: collapsed mode'da search input gizli, kullanıcı
              filtreleyemiyor. Bu ikon tıklanınca sidebar açılıyor ve
              arama input'una focus veriyor — single-click flow. */}
          <button
            onClick={() => {
              onToggleCollapse?.();
              // Defer focus until after the collapse transition tick.
              setTimeout(() => {
                const el = document.querySelector<HTMLInputElement>('[data-sidebar-search]');
                el?.focus();
              }, 50);
            }}
            aria-label="Sohbet ara (sidebar'ı aç)"
            title="Sohbet ara"
            className="flex h-9 w-9 items-center justify-center rounded-lg text-ak-text-secondary hover:bg-ak-surface-2 hover:text-ak-primary transition-colors"
          >
            <svg
              className="h-3.5 w-3.5"
              fill="none"
      ...
      <div
        className={cn('flex-1 overflow-y-auto', collapsed ? 'px-2 py-2 space-y-1' : 'px-2 py-2')}
      >
        {filtered.length === 0 && !collapsed && (
          <div className="flex flex-col items-center gap-2 px-3 py-8 text-center">
            <svg
              className="h-8 w-8 text-ak-text-tertiary/40"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={1}
        ...
        <div className="border-t border-ak-border-subtle px-3 py-2">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-medium text-ak-text-primary">
      ...
      <div
        className={cn(
          'border-t border-ak-border-subtle px-3 py-2',
