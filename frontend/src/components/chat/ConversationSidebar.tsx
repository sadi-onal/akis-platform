...
export function ConversationSidebar({
  ...
  const isEngineerActive = location.pathname.startsWith('/engineer');
      ...
      <div className={cn('space-y-0.5 px-2 py-2', collapsed && 'flex flex-col items-center')}>
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-2.5 rounded-lg text-[13px] font-medium transition-colors',
            collapsed ? 'h-9 w-9 justify-center' : 'w-full px-3 py-2',
            isEngineerActive ? 'bg-ak-surface-2 text-ak-text-primary' : 'text-ak-text-secondary hover:bg-ak-surface-2/50 hover:text-ak-text-primary',
          )}
          aria-label="Mühendis"
        >
          <svg className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
