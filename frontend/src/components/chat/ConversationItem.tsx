...
export function ConversationItem({
  ...
  // Single concise label so screen readers announce the card once instead of
  // concatenating every visible span (which was producing duplicated reads —
  // status + title + repo + file-count + time, with the title's `title` attr
  // adding the same text again).
  const ariaLabel = [
    statusLabel,
    displayTitle,
    repoFullName,
    fileCount > 0 ? `${fileCount} dosya` : null,
  ...
  return (
    <div className="group relative">
      ...
      >
        ...
        {/* Info row */}
        <div aria-hidden="true" className="flex items-center gap-1 pl-4">
