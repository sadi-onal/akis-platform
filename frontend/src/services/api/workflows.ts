    ...
    switch (msg.type) {
      ...
      case 'scribe_completed': {
        ...
        if (PRE_APPROVAL_STAGES.indexOf(stage) === -1) {
          planStatus = 'approved';
        } else if (stage === 'critic_reviewing_spec') {
          planStatus = 'reviewing';
        } else if (stage !== 'awaiting_approval' && stage !== 'awaiting_critic_resolution') {
          // Pre-approval but not yet actionable (e.g. scribe_generating).
          // Show the plan card without approval buttons.
          planStatus = 'edited';
        }
        const embeddedPlan = scribeSpec
          ? {
              ...
              version: 1,
              status: planStatus as PlanStatus,
              spec: scribeSpec,
