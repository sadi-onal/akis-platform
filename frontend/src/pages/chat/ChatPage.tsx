...
function workflowToListItem(w: Workflow): ConversationListItem {
  ...
  return {
    ...
    prUrl: undefined,
    engineerSessionId: w.engineerSessionId,
  };
}
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-ak-bg" role="application" aria-label="AKIS Chat">
      ...
      <div className={cn('flex min-w-0 flex-1 flex-col min-h-0', 'pt-[52px] md:pt-0')}>
        ...
        {activeWorkflow?.engineerSessionId && (
          <div className="flex items-center gap-3 border-b border-[#07D1AF]/20 bg-[#07D1AF]/[0.06] px-4 py-2 text-[12px]">
            <svg className="h-4 w-4 flex-shrink-0 text-[#07D1AF]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z" />
            </svg>
            <span className="flex-1 text-ak-text-secondary">
              Bu sohbet bir <span className="font-semibold text-[#07D1AF]">Mühendis Modu oturumuna</span> ait.
            </span>
            <button
              onClick={() => navigate(`/engineer/session/${activeWorkflow.engineerSessionId}`)}
              className="rounded-md border border-[#07D1AF]/30 bg-[#07D1AF]/10 px-2.5 py-1 text-[11px] font-medium text-[#07D1AF] transition-colors hover:bg-[#07D1AF]/20"
            >
